...
| -------------------------------------------- | ------------------------------------------------------ | --------------------------------------------------- | :----------------------------------------------------------- |
| id="md_id"<br> name="**m1**"<br>  value=null | id="md_id"<br> name=null<br>  value=null               | id="md_id"<br> name="**m1**"<br>  value=See Details | For numeric SdsTypeCode, the value for **m1** is 0. For string SdsTypeCode, the value for **m1** is an empty string. For both, the metadata value is returned, along with UOM and SdsTypeCode from the asset type. |
...
| Not present                                  | id="md_id"<br/> name="**m5**"<br/>  value=30           | id="md_id"<br/> name="**m5**"<br/>  value=30        | m5 appears on the default shape with value =30, along with UOM and SdsTypeCode from the asset. Since this metadata value does not exist on the asset type, the metadata value on the default shape takes the value of the asset. |
